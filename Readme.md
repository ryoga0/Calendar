...
`$env:FIREBASE_CREDENTIALS_PATH=".\service-account.json"` の  
`.\service-account.json` は、**自分で中身を書く設定ファイルではなく、Firebase / Google Cloud からダウンロードする管理者用の鍵ファイル**です。

このファイルが必要な理由:

- `scripts/seed_firestore_departments.py` は Python から Firestore を更新する
- そのとき通常のフロントエンド用 `.env` ではなく、**管理者権限の認証情報**が必要になる
- その認証情報が `service-account.json`

どうやって取得するか:

1. Firebase Console を開く
2. 左下の歯車から `プロジェクトの設定` を開く
3. `サービス アカウント` タブを開く
4. `Firebase Admin SDK` を開く
5. `新しい秘密鍵を生成` を押す
6. ダウンロードされた JSON を保存する

どこに置くか:

- README の例では、`Calendar/` 直下に置く前提です
- つまり次のような配置です
...
この配置にすると、PowerShell で `Calendar/` 直下にいる状態でダウンロードしたファイルを`service-account.jsonに変更してください`
...
`.\service-account.json` の意味:

- `.\` は「今いるフォルダ」を意味します
- つまり `Calendar\service-account.json` を見に行く、という意味です

もし別の場所に置くなら、その実際のパスを書く必要があります。

注意:

- この JSON には秘密鍵が入っているので、**Git にコミットしない**
- 公開リポジトリに入れない
- 他人に送らない
- seed を使わないなら必須ではない
...
このスクリプトは `DEPARTMENT_SEEDS` を基準に Firestore の `departments` を同期します。  
既存データがあっても、同じ診療科は更新し、足りない診療科は追加します。  
seed に無い既存診療科は削除しません。
...
- Spark プランで動かすため、予約更新と管理者更新は Firestore 直書きです
- 監査ログも Firestore に直接保存します
